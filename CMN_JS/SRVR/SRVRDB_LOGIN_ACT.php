...
ORDER BY 
  ...
  ua.`fkImpiego` ASC 
      ");
      /*
       *  ui.`idImpiego` IS NULL ASC,  -- prima i record con impiego, poi la riga ANAG NULL
       *  ds.`codiceSito` ASC,         -- dentro gli impieghi, ordina per sito
       *  ua.`fkImpiego` ASC;          -- dentro il sito, ordina per abilitazione" 
       */ 
  ...
  function ActForcePWD($FROM, $_post)
  {
     ...
     */
    var_dump($_post);
    if (empty($CF = $_post['codiceFiscale']) || empty($PWD = $_post['PWD']))
    {
  ...
  function ActRegistrazione($FROM, $_post)
  {
    ...
    try 
    {
      ...
      "INSERT INTO `sacs`.`Utenti`
        (
          `idUtente`, `nome`, `cognome`, `codiceFiscale`, `username`, `passwordHash`, `forzaCambioPassword`, 
          `isManagementRoot`, `isMultisitoRoot`, `dataRegistrazione`, `dataCessazione`
        )
      VALUES 
        (
          (SELECT CONCAT('TMP',COUNT(*)+1) FROM `sacs`.`Utenti` B WHERE `idUtente` LIKE 'TMP%%'), 
                ?, ?, ?, ?, ?, 1, 0, 0, CURDATE(), NULL
        )
      ");
      $Qry->set_Parm($_post["USRNAME"]);
      ...
      $Qry->set_Parm(password_hash($_post["PWD"], PASSWORD_BCRYPT));
      $Qry->set_Type("I");
      $Qry->set_NField(0);
      $QryArr->push_Query($Qry);
      $PMTxt="====NUOVA REGISTRAZIONE====<br>".
        "Nome: {$_post["USRNAME"]}<br>".
        "Cognome: {$_post["USRCOGNOME"]}<br>".
        "Codice Fiscale: {$_post["USRCF"]}<br>".
        "Username: {$_post["USR_ID"]}<br>".
        "Data: ".date("d/m/Y")."<br>".
        "Necessario assegnare SITO ed ABILITAZIONE.<br>";
      $QryPM = new SqlQuery();
      $QryPM->set_SqlTxt(  
    ...
    if ($Qry->get_NRec() == 0) 
    {
      ...
      WriteErr($FROM, $THIS_FUNCTION, $THIS_FILE, __LINE__-3, "{$THIS_FUNCTION} got error inserting values 0 record added");
      require "../MSG/SYS_ERR.html";
    ...
    if ($Qry->get_NRec() > 1) 
    {
      ...
      WriteErr($FROM, $THIS_FUNCTION, $THIS_FILE, __LINE__-3, "{$THIS_FUNCTION} got error inserting values. More than 1 record added");
      require "../MSG/SYS_ERR.html";
