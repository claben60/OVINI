<?php
  class SqlQuery 
  {
    ...
    // Methods
    function set_Type(string $Type): void
    {
      $this->Type = $Type;  
    } 
    ...
    {
       $this->ArrParms = is_array($ArrParms)? $ArrParms : [$ArrParms];
    }    
    ...
    /**
     * Trasforma lo stato dell'oggetto in array per WriteLog/WriteErr
     */
    function toLogArray()
    {
      return [
        "Type"     => $this->get_Type(),
        "SqlTxt"   => $this->get_SqlTxt(),
        "ArrParms" => $this->get_ArrParms(),
        "RC"       => $this->get_RC(),
        "NField"   => $this->get_NField(),
        "NRec"     => $this->get_NRec(),
        "ArrRec"   => $this->get_ArrRec() // Se l'array di record è enorme, si può limitare o serializzare
      ];
    }
  }
  ...
  class SqlQueryArr
  {
    ...
    /**
     * Trasforma l'intero vettore di query in una struttura array tracciabile
     */
    function toLogArray()
    {
      $queriesLog = [];
      foreach ($this->QueryArr as $idx => $qry) {
        if ($qry instanceof SqlQuery) {
          $queriesLog["Query_#{$idx}"] = $qry->toLogArray();
        }
      }

      return [
        "NElements"   => $this->get_NElements(),
